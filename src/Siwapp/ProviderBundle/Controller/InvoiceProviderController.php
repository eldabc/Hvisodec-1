    		...
    		if (!$providerEntity) {
    			...
    			foreach ($entity->getItems() as $item){
    			    $changeset = $uow->getEntityChangeSet($item);
						// echo "changeset['quantity'][0] ". $changeset['quantity'][0];
						// echo "<br>";
						// echo "changeset['quantity'][1] ". $changeset['quantity'][1];
						$stock = $item->getStock();
        			    if($changeset['quantity'][0] != $changeset['quantity'][1]){
        			        ...
        			        if ($changeset['quantity'][0] < $changeset['quantity'][1]){
								...
								// echo "<br>";
								// echo $changeset['quantity'][1] - $changeset['quantity'][0];
        			            $item->setStock(intval($item->getStock() + $diff));
        			        }else{
								$diff = ($changeset['quantity'][0] - $changeset['quantity'][1]);
								// echo "<br>";
								// echo $changeset['quantity'][0] - $changeset['quantity'][1];
								$item->setStock($item->getStock() - $diff);
        			        }
							
						}

						if($changeset['description'][0] != $changeset['description'][1]){
							$item->setDescription($changeset['description'][1]);
						}
						// echo "<br>";
						// echo "diff ". $diff;
						$em->persist($item);
